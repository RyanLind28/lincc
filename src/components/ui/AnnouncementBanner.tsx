...
export function AnnouncementBanner() {
  ...
  const config = announcement
    ? typeConfig[announcement.type as keyof typeof typeConfig] || typeConfig.info
    : null;
  ...
  // Render a zero-height wrapper always to avoid CLS; only expand when announcement exists
  return (
    <div
      className="overflow-hidden transition-all duration-300 ease-out"
      style={{ maxHeight: announcement ? '200px' : '0px' }}
    >
      {announcement && config && Icon && (
