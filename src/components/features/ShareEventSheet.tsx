...
export function ShareEventSheet({ isOpen, onClose, event }: ShareEventSheetProps) {
  const { user } = useAuth();
  ...
  const handleSendToFriend = async (friend: FollowProfile) => {
    ...
    try {
      ...
      try {
        ...
        if (convoResult.success && convoResult.data) {
          const dmResult = await sendDirectMessage(
            convoResult.data.id,
            user.id,
            `Check out this event: ${event.title} at ${event.venue_name}!`,
            'event_share',
            { event_id: event.id }
          );
      ...
      const result = await createNotification(
        ...
        'Someone shared an event with you',
        `${event.title} at ${event.venue_name}`,
        { event_id: event.id, sender_id: user.id }
      );
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={onClose} title="Share Event" height="half">
